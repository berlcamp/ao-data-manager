...
export default function AddStickyModal({ item, hideModal }: ModalProps) {
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from('adm_tracker_stickies').insert({
      tracker_id: item.id,
      color,
      note,
  ...
  return (
    <div
      ref={wrapperRef}
      className="z-40 fixed top-0 left-0 w-full h-full outline-none overflow-x-hidden overflow-y-auto bg-gray-900 bg-opacity-50">
