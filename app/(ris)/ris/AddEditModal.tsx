...
export default function AddEditModal({ hideModal, editData }: ModalProps) {
  ...
  useEffect(() => {
    // Fetch vehicles
    ;(async () => {
      const { data } = await supabase.from('adm_ris_vehicles').select()
      setVehicles(data)
    })()

    // Fetch departments
    ;(async () => {
      const { data } = await supabase.from('adm_ris_departments').select()
      setDepartments(data)
    })()
